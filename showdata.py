...
def data_update(filename):
    ...
    if text != old_text:
        tkt.Text(tkt.Canvas(data_root, 540, 960, 0, 0), 0, 1, 540, 960, text=(text, text), radius=5, borderwidth=1, font=myfort, image=tkt.PhotoImage('miku.png'), color_fill=tkt.COLOR_NONE, justify='left', color_text=('#ffffff', '#ffffff', '#ffffff', '#ffffff'), read=True)
        old_text = text
